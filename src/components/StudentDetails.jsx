  ...
  return (
    ...
    <div className="row">
       ...
       <div className="col-sm-12 col-md-4">
           <span> {props.student.sname}</span>
           <span> {props.student.email}</span>
            <span>{props.student.rollno}</span>
            
            <span>{props.student.prn}</span>
            <span> {props.student.course}</span>
       </div>
       <div>
          <Link to={`/view/${props.student.rollno}`}>
           <button type="button" name="btn" className="btn btn-success">View</button>
           </Link> &nbsp;&nbsp;&nbsp;
           <button type="button" name="btn" className="btn btn-danger" onClick={()=>{deletestudent(props.student.rollno)}}>Delete</button>&nbsp;&nbsp;&nbsp;
           <Link to={`/edit/${props.student.rollno}`} state={{empedit:props.student}}>
